...
public class TypeFilter {
    ...
    public static final TypeFilter IMPOSSIBLE =
            new TypeFilter(MultiThreadAnalysisUtil.getErrorClass(),
                           Collections.singleton(MultiThreadAnalysisUtil.getErrorClass())) {
                @Override
    ...
    public TypeFilter(IClass isType, Set<IClass> notTypes) {
        ...
        if (MultiThreadAnalysisUtil.getObjectClass().equals(this.isType)) {
            throw new IllegalArgumentException();
        }
    ...
    private TypeFilter(Set<IClass> isTypes, Set<IClass> notTypes) {
        ...
        if (MultiThreadAnalysisUtil.getObjectClass().equals(isType)) {
            throw new IllegalArgumentException();
        }
    ...
    private boolean isAssignableFrom(IClass c1, IClass c2) {
        ...
        if (notTypes == null) {
            return MultiThreadAnalysisUtil.getClassHierarchy().isAssignableFrom(c1, c2);
        }
...
//            }
    ...
    public static TypeFilter create(TypeReference isType, Set<IClass> notTypes) {
        return create(MultiThreadAnalysisUtil.getClassHierarchy().lookupClass(isType),
                      notTypes);
    }
        ...
        if (filter.isTypes != null) {
            for (IClass t1 : filter.isTypesAsSet()) {
                for (IClass t2 : filter.isTypesAsSet()) {
                    ...
                    if (t1.isInterface()
                            && t2.isArrayClass()
                            && !(t1.equals(MultiThreadAnalysisUtil.getCloneableInterface()) || t1.equals(MultiThreadAnalysisUtil.getSerializableInterface()))) {
                        // t2 is an array, and t1 is an interface that an array can't implement
                        return true;
                    }
                    if (t2.isInterface()
                            && t1.isArrayClass()
                            && !(t2.equals(MultiThreadAnalysisUtil.getCloneableInterface()) || t2.equals(MultiThreadAnalysisUtil.getSerializableInterface()))) {

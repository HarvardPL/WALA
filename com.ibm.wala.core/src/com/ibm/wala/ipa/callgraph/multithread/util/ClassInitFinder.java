...
public class ClassInitFinder {
    ...
    public static IClass getRequiredInitializedClasses(SSAInstruction i) {
        ...
        if (i instanceof SSANewInstruction) {
            ...
            if (!ins.getConcreteType().isArrayType()) {
                ...
                return MultiThreadAnalysisUtil.getClassHierarchy().lookupClass(rf);
            }
        }
        ...
        if (i instanceof SSAInvokeInstruction) {
            ...
            if (ins.isStatic()) {
                IMethod callee = MultiThreadAnalysisUtil.getClassHierarchy().resolveMethod(ins.getDeclaredTarget());
                if (callee == null) {
        ...
        if (i instanceof SSAPutInstruction) {
            ...
            if (ins.isStatic()) {
                IField f = MultiThreadAnalysisUtil.getClassHierarchy().resolveField(ins.getDeclaredField());
                if (f == null) {
    ...
    public static List<IMethod> getClassInitializersForClass(IClass klass) {
        if (klass != null) {
            IClass objectClass = MultiThreadAnalysisUtil.getClassHierarchy().getRootClass();
